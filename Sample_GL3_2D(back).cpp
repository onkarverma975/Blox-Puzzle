...
void CubeToppleNorth(){
	cube1.pos = glm::vec3(  cube1.back.x,
							cube1.back.y + cube1.scale.y + cube1.scale.z*sin(cube1.thetay*M_PI/180.0f)*sqrt(2),
							floor_grey.scale.z                 + cube1.scale.z*cos(cube1.thetay*M_PI/180.0f)*sqrt(2)
						 );
	cube1.thetay += cube1.dr;
	if(cube1.thetay == 55){
		cube1.pos = glm::vec3(cube1.back.x,cube1.back.y+1,cube1.back.z);	
		cube1.thetay=-45;
		cube1.toppling=0;
	}
}
...
void CubeToppleWest(){
	cube1.pos = glm::vec3(  cube1.back.x - cube1.scale.y + cube1.scale.z*sin(cube1.thetax*M_PI/180.0f)*sqrt(2),
							cube1.back.y, 
							floor_grey.scale.z                 + cube1.scale.z*cos(cube1.thetax*M_PI/180.0f)*sqrt(2)
						 );
	cube1.thetax += cube1.dr;
	if(cube1.thetax == -55){
		cube1.pos = glm::vec3(cube1.back.x-1,cube1.back.y,cube1.back.z);	
		cube1.thetax=-45;
		cube1.toppling=0;
	}
}
...
void CubeActivateTopple(int dir){
	if(cube1.toppling !=0)
		...
		cube1.toppling = dir;
		cube1.back = cube1.pos;
		cube1.dr=cube1.speed;
		cube1.thetay=-45;
	}
	else if(dir==2){
		cube1.toppling = dir;
...
void createRectangle ()
{
	...
	cube1.scale = cube1.scale = glm::vec3(0.5,0.5,0.5);
	cube1.pos = cube1.pos = glm::vec3(0,0,floor_grey.scale.z+cube1.scale.z);
	cube1.speed =	 10;
	...
	cube1.color.r=1;
	cube1.color.g=162.0f/255.0f;
	cube1.color.b=200.0f/255.0f;
	static const GLfloat color_buffer_data_cube1[] = {
		cube1.color.r,  cube1.color.g,  cube1.color.b,
		cube1.color.r,  cube1.color.g,  cube1.color.b,
		cube1.color.r,  cube1.color.g,  cube1.color.b,
		cube1.color.r,  cube1.color.g,  cube1.color.b,
		cube1.color.r,  cube1.color.g,  cube1.color.b,
	...
	floor_grey.line = floor_orange.line = cube2.line =cube1.line = create3DObject(GL_TRIANGLES, 13*3, vertex_buffer_data, color_buffer_data_line, GL_LINE);
	
	floor_orange.object = create3DObject(GL_TRIANGLES, 13*3, vertex_buffer_data, color_buffer_data_orange, GL_FILL);

	cube1.object = create3DObject(GL_TRIANGLES, 13*3, vertex_buffer_data, color_buffer_data_cube1, GL_FILL);

	cube2.object = create3DObject(GL_TRIANGLES, 13*3, vertex_buffer_data, color_buffer_data_cube1, GL_FILL);
...
void draw (GLFWwindow* window, float x, float y, float w, float h, int doM, int doV, int doP)
{
    ...
    glm::mat4 translateCube1 = glm::translate (cube1.pos);        // glTranslatef
    glm::mat4 rotateCube1X = glm::rotate((float)(-(cube1.thetax+45)*M_PI/180.0f), glm::vec3(0,-1,0));
    glm::mat4 rotateCube1Y = glm::rotate((float)(-(cube1.thetay+45)*M_PI/180.0f), glm::vec3(1,0,0));
    glm::mat4 myScalingMatrix = glm::scale(glm::mat4(1.0f),cube1.scale);
    Matrices.model *= (translateCube1 * rotateCube1X * rotateCube1Y * myScalingMatrix);
	MVP = VP * Matrices.model;
    glUniformMatrix4fv(Matrices.MatrixID, 1, GL_FALSE, &MVP[0][0]);
